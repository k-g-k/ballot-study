...
export function LogoWithTooltip({
  ...
  return (
    <span className="relative group shrink-0 inline-block leading-none">
      <Logo u={u} size={size} />
      <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-[6px] hidden group-hover:block bg-ink text-ink-inverse font-body text-xs px-[8px] py-[4px] rounded-control whitespace-nowrap z-20 pointer-events-none">
        {label ?? u.name}
      </span>
    </span>
  );
