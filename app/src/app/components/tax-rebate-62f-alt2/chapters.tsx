...
import { StanceChip } from "../tax-rebate-62f/accounts";
import {
  RC,
  SOURCES,
  PROSE,
  ...
  return (
    <Chapter id="sides" question="What are the arguments?">
      <div className="flex flex-col gap-[28px] sm:gap-[40px]">
        ...
        {/* The Legislature's own pair, last. "Where do they agree?" above is
            about the two campaigns: the data says so in its own words, "both
            sides describe the mechanism the same way", and a committee report
            sitting between the campaigns and that question leaves the reader
            unsure who "they" are. It is also the order things happened in: the
            campaigns argued, the committee reviewed, the question went to the
            ballot.

            Not merged with the campaigns either. A committee recommending no
            action is not the same thing as endorsing the No campaign, and
            stacking them in the same columns would say it was. */}
        <Span>
          <h3 className="font-display font-medium text-xl sm:text-2xl lg:text-3xl tracking-display text-ink max-w-[20ch] text-balance">
            What did the Legislature say?
          </h3>
          {/* The record, before the reports rather than after them. It is what
              both reports came out of, so it belongs where a reader can take it
              before reading either, not as a footnote once they have. */}
          ...
          {[MAJORITY_REPORT, MINORITY_REPORT].map((r) => (
            <div key={r.kind} className="max-w-[74ch]">
              ...
              <p className="flex items-center gap-[10px]">
                ...
                <StanceChip stance={r.kind === "minority" ? "endorse" : "oppose"} />
              </p>
              {/* The report itself, closed. Carried verbatim it runs to nine
                  ...
                  signed it sits outside, below: that is the page naming these
              ...
              <div className="mt-[18px]">
                <Disclosure label="View full text">
              ...
              <dl className="flex flex-wrap gap-x-[36px] gap-y-[10px] mt-[22px]">
                {r.signers.map((g) => (
                  <div key={g.chamber}>
...
export function WhereThisComesFrom() {
  return (
    ...
    >
      {/* The two documents everything else on the page is downstream of: what
          the Attorney General says the measure does, and what the measure
          actually says. Closed by default, because most readers want the page's
          account of them and the rest want the words themselves. */}
      <Span>
        <Disclosure label="The Attorney General's summary">
          <p className="font-body text-xs text-ink-muted leading-[1.6] mb-[14px]">
            {SUMMARY.provenance}
          </p>
          <div className="flex flex-col gap-[12px] border-l-2 border-official pl-[16px] sm:pl-[20px]">
        ...
        <Disclosure label="Full text of the proposed law">
          <p className="font-body font-semibold text-base text-ink leading-[1.5] mb-[16px]">
            {FULL_TEXT_TITLE}
          </p>
          <ol className="flex flex-col gap-[18px]">
            {FULL_TEXT.map((sec) => (
              <li key={sec.section}>
                <p className="font-body font-semibold text-2xs uppercase tracking-[0.08em] text-ink-faint mb-[5px]">
                  {sec.section}
                </p>
